...
export const useThumbnail = () => {
  const createImage = async (canvas: HTMLCanvasElement, filename: string) => {
    ...
    const toBlob = (incomingCanvas: HTMLCanvasElement): Promise<File> => {
      return new Promise((resolve, reject) => {
        incomingCanvas.toBlob((blob) => {
          if (blob)
            resolve(
  ...
  const createCanvas = useCallback(
    async (pdf: PDFJS.PDFDocumentProxy, filename: string) => {
  ...
  const generatePdfThumbnail = useCallback(
    async (file: File, filename: string, extension: string) => {
      if (extension !== 'pdf') return null;
      PDFJS.GlobalWorkerOptions.workerSrc = `${window.location.origin}/js/thirdparty/pdf.worker.min.js`;

      const fileReader = new FileReader();

      const arrayBuffer: () => Promise<Uint8Array> = () =>
  ...
  const generateImageThumbnail = async (file: File) => {
    ...
    const imageBase64 = await resizeFile(file, {
      ...options,
      outputType: 'base64',
    });
  ...
  const generateVideoThumbnail = useCallback(
    async (file: File, fileName: string) => {
      return generateVideoThumbnails(file, 1, 'base64')
        .then(async (snapshots) => {
          const snapshot = snapshots[0];
          if (snapshot) {
            const base64Reponse = await fetch(snapshot).catch((error) =>
            ...
            if (base64Reponse) {
              ...
              const resizedImage = await generateImageThumbnail(newFile);
              return resizedImage;
            }
          }
        })
  ...
  const generators: {
    ...
    () => ({
      pdf: (file, filename, extension) =>
        generatePdfThumbnail(file, filename, extension),
