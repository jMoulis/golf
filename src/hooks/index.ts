...
export const generateVideoThumbnails = async (videoFile: File, numberOfThumbnails: number, type: string): Promise<string[]> => {
  ...
  return type !== "url" ? new Promise(async (resolve, reject) => {
    ...
    reject({ code: CODE_ERRORS.SYSTEM, message: 'Unable to generate thumbnails' })
  })
    : new Promise(async (resolve, reject) => {
      ...
      reject({ code: CODE_ERRORS.SYSTEM, message: 'Unabale to get video duration from the file' })
    });
};
...
const getVideoThumbnail = (file: File | string, videoTimeInSeconds: number): Promise<string> => {
  return new Promise((resolve, reject) => {
    if ((file as File)?.type?.match("video")) {
      ...
      reject({ code: CODE_ERRORS.SYSTEM, message: 'Unable to get video thumbnail' })
    }
  });
};
    ...
    try {
      ...
      videoPlayer.addEventListener('loadedmetadata', () => {
        ...
        if (videoPlayer.duration < seekTo) {
          reject({ code: CODE_ERRORS.TOO_SHORT, message: 'The video is to short' })
          return;
        }
...
export const generateVideoThumbnailViaUrl = (urlOfFIle: string, videoTimeInSeconds: number): Promise<string> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      reject({ code: CODE_ERRORS.SYSTEM, message: 'Unable to generate thumbnail from URL' })
    }
  })
}
  ...
  return new Promise((resolve, reject) => {
    try {
      if (videoFile) {
        if ((videoFile as File)?.type?.match("video")) {
          ...
          importFileandPreview(videoFile as File).then((url) => {
            generateVideoDurationFromUrl(url).then((res) => {
              resolve(res);
            }).catch((error) => reject({ code: CODE_ERRORS.SYSTEM, message: error.message }))
          });
        } else {
        ...
        reject({ code: CODE_ERRORS.SYSTEM, message: "Cannot generate video duration for this video file." });
      }
    } catch (error: any) {
      reject({ code: CODE_ERRORS.SYSTEM, message: error.message })
    }
